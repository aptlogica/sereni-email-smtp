...
func TestGenerateOTPAndValidators(t *testing.T) {
	...
	if !IsValidEmail("a@b.com") || IsValidEmail("bad") {
		t.Fatalf("email validator failed")
	}

	if !IsValidEmailList([]string{"x@y.com", "a@b.com"}) {
		t.Fatalf("email list validation failed")
	}
}

func TestTemplateRenderAndManagement(t *testing.T) {
	es := NewEmailService("h", 25, "u", "p", "from@x", 5)
	...
	et := EmailTemplate{Subject: "S", HTMLBody: "<b>{{.who}}</b>", TextBody: "{{.who}}"}
...
func TestSendTemplateEmailAndTransactional(t *testing.T) {
	...
	req := &EmailRequest{To: []string{"bad"}, Template: "otp_template"}
...
// mockClient implements smtpClient for testing SendEmail via Dial injection
...
func TestSendEmail_DialAndClientErrors(t *testing.T) {
	...
	// Dial error
	es.Dial = func(addr string) (smtpClient, error) { return nil, errors.New("dial fail") }
	...
	es.Dial = func(addr string) (smtpClient, error) { return mock, nil }
	if err := es.SendEmail([]string{"a@b.com"}, "s", "b", false); err == nil {
...
func TestSendBulkEmail_FailuresAggregated(t *testing.T) {
	es := NewEmailService("h", 25, "u", "p", "from@x", 2)
...
func TestSendTransactionalEmail_SuccessAndTemplateFlow(t *testing.T) {
	...
	req := &EmailRequest{To: []string{"a@b.com"}, Subject: "s", Body: "b"}
	if err := es.SendTransactionalEmail(req); err != nil {
